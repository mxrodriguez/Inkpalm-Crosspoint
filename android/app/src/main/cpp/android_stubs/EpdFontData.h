// EpdFontData.h — Font data structures for CrossPoint Android port
// Direct copy from lib/EpdFont/EpdFontData.h — no modifications needed
// These are pure data structures with no platform dependencies
...
/// Rotated-90CW variant of centerOver.
...
/// edge and the top of the base glyph.
...
typedef struct {
  ...
  const uint16_t* glyphToGroup;               ///< Per-glyph group ID
  const EpdKernClassEntry* kernLeftClasses;   ///< Sorted left-side class map
  const EpdKernClassEntry* kernRightClasses;  ///< Sorted right-side class map
  ...
  const EpdLigaturePair* ligaturePairs;  ///< Sorted ligature pair table
